...
export default function LearningPage() {
  const [chat, setChat] = useState([{ role: "assistant", text: "Hi! Need a hint? Try arranging the blocks to print a name." }]);
        ...
        <Card className="shadow-md">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <Mascot className="w-12 h-12" mood={lessonDone ? "cheer" : "idle"} />
              <p className="font-bold">AI Tutor</p>
            </div>
            <div className="mt-3 space-y-2 max-h-[320px] overflow-auto">
              {chat.map((m, i) => (
                <div key={i} className="rounded-lg p-3 bg-muted text-sm">{m.text}</div>
              ))}
            </div>
            <div className="mt-3 flex gap-2">
              <Button size="sm" variant="outline" onClick={() => setChat([...chat, { role: "assistant", text: "Hint: Start by reading the user's name." }])}>Hint</Button>
              <Button size="sm" onClick={() => setChat([...chat, { role: "assistant", text: "Great thinking!" }])}>Ask</Button>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <Card className="shadow-md">
          <CardContent className="p-4">
            ...
            <p className="text-sm text-muted-foreground">Drag code blocks into the correct order to read a name and print it.</p>
            <div className="mt-4 grid sm:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl p-3 bg-accent/10 min-h-[180px]" onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <Button variant="outline" onClick={() => setArr([])}>Reset</Button>
            </div>
          </CardContent>
        </Card>
      </div>
